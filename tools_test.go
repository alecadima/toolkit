...
func TestTools_CreateDirIfNotExist(t *testing.T) {
	...
	if _, err := os.Stat("./testdata/myDir"); os.IsNotExist(err) {
		t.Error(err)
	}
...
var slugifyTests = []struct {
	...
	{name: "complex string", s: "Now is the time for all GOOD men! + fish & such &ˆ123", expected: "now-is-the-time-for-all-good-men-fish-such-123", errorExpected: false},
...
func TestTools_Slugify(t *testing.T) {
	...
	for _, e := range slugifyTests {
		...
		if err != nil && !e.errorExpected {
			t.Errorf("%s: error expected when none expected: %s", e.name, err.Error())
		}
